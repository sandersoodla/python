...
pLaius = tkinter.Scale(window, from_=0, to=100, orient="horizontal")
...
def motion(event):
    if b1 == "all":
        ...
        if xvana != None and yvana != None and riist == "p":
            objectlist.append(event.widget.create_oval(event.x, event.y, event.x+1, event.y+1, fill=värv[1], width=pLaius.get()))
            print(len(objectlist))
        if xvana != None and yvana != None and riist == "k":
            event.widget.create_rectangle(xvana, yvana, event.x, event.y, fill='white', width=20)
        xvana = event.x
        yvana = event.y

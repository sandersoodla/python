...
tahvel = tkinter.Canvas(window, width= 2000, height= 2000, background='white', cursor="pencil")
...
def motion(event):
    ...
    if b1 == "üleval":
        event.widget.create_polygon(event.x, event.y, event.x+1, event.y+1, outline=värv[1], width=pLaius.get(), tags="eelvaade")
        tahvel.move(eelvaade, event.x, event.y)
